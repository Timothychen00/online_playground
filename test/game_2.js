...
} function check_can_move() { let condition = 0; for (let i = 0; i < 4; i++) { for (let j = 0; j < 3; j++) { if (block_num[i][j] == block_num[i][j + 1] || block_num[i][j] == 0 || block_num[i][j + 1] == 0) { condition = 1; } } } for (let i = 0; i < 3; i++) { for (let j = 0; j < 4; j++) { if (block_num[i][j] == block_num[i + 1][j] || block_num[i][j] == 0 || block_num[i + 1][j] == 0) { condition = 1; } } } return condition; } let block_width = 82; let block_radius = 13; let animation_queue = []; //set the colors
...
} function preload() { sans = loadFont("Sans.ttf"); } function start_game(reset) {
...
function mouseClicked() {
    ...
    if (push == 1) { //popMatrix();
        push = 0;
    } if (key == 'd') { handle_movement(- 1, 0); } else if (key == 's') { handle_movement(0, - 1); } else if (key == 'a') { handle_movement(1, 0); } else if (key == 'w') { handle_movement(0, 1); } //random_block();
    console.log(available_movements); if (check_can_move() == 1) { if (available_movements > 0) { random_block(); } }
