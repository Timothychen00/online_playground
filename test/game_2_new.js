...
//y->i
//x->j
let colors = ["#FFFFFF", "#ECE4DB", "#ECE1CC", "#E9B582", "#E99A6D", "#E78367", "#E56948", "#E8D180", "#E8CD72"]; let text_colors = ["#FFFFFF", "#756E66", "#756E66", "#F8F6F2", "#F8F6F2", "#F8F6F2", "#F8F6F2", "#F8F6F2", "#F8F6F2"]; let score = 0; let steps = 0; let push = 0; let lose = 0; let available_movements = 0; let proress = 0; //animation
let appear = [- 1, - 1]; let animation_start = 0; function show_score() {
     //rect();
     fill("#CAC1B5"); rect(535 - 80 - 5, 70, 80, 80, 10); fill("#F8F6F2"); //println(score);
     //text(,100,100);
     textSize(20); text("Score", 535 - 80 - 5, 70 - 20); text(str(score), 535 - 80 - 5, 70 + 20);
}
function check_can_move() {
     let condition = 0; for (let i = 0; i < 4; i++) {
          for (let j = 0; j < 3; j++) {
...
let count = 0; function show_block() {
               ...
               if (block_num[i][j] != 0) {
                    let num = int(log(block_num[i][j]) / log(2)); if (appear[0] != - 1) {
                         if (int(appear[0]) == i && int(appear[1]) == j) {
                              ...
                              let now = millis(); if (now - animation_start < 250) {
                                   translate(x[j], y[i]); scale(count * 0.15); translate(- x[j], - y[i]); count++;
                              }
                              else {
                                   count = 0; appear[0] = - 1; appear[1] = - 1; animation_start = 0;
                              }
                              scale(1); //popMatrix();
                         }
                    }
                    ...
                    text(block_num[i][j], x[j], y[i]); if (appear[0] != - 1 && int(appear[0]) == i && int(appear[1]) == j) {
                         pop();
                    }
               }
          }
     ...
     if (lose == 1) {
          fill("#e0d296"); rect(400, 325, 390, 390, 25); fill("#FFFFFF"); text("you lose!", 400, 325);
     }
}
//initial the main block
function generate_board() {
     fill("#B9ADA1"); rect(400, 325, 390, 390, 25); fill("#CAC1B5"); stroke("#B9ADA1"); for (let i = 0; i < 4; i++) {
          for (let j = 0; j < 4; j++) {
               rect(x[i], y[j], 82, 82, 13);
          }
     }
}
function non_empty(certain, dx, dy, start, end) {
     //dx,dy
     let return_value = - 1; if (dy == 0) {
          console.log("pppp"); for (let i = start; i != end; i += dx) {
               //move right
          ...
          //move down
          console.log("----"); for (let i = start; i != end; i += dy) {
               if (block_num[i][certain] != 0) {
                    console.log(block_num[i][certain]); if (i == start) continue; return i;
               }
          }
     }
     //else if(dx>0){
...
function handle_movement(dx, dy) {
     if (dx < 0) {
          console.log("movement dx"); for (let j = 0; j < 4; j++) {
               for (let i = 3; i != - 1; i += dx) {
                    let next = non_empty(j, - 1, 0, i, - 1); if (next != - 1) {
                         //not empty
     ...
     if (dy < 0) {
          console.log("movement dy"); for (let j = 0; j < 4; j++) {
               for (let i = 3; i >= 0; i--) {
                    let next = non_empty(j, 0, - 1, i, - 1); if (next != - 1) {
                         //not empty
     ...
     if (dx > 0) {
          ...
          for (let j = 0; j < 4; j++) {
               for (let i = 0; i != 4; i++) {
                    let next = non_empty(j, dx, dy, i, 4); if (next != - 1) {
                         //not empty
                         if (block_num[j][i] == block_num[j][next]) //merge
                         {
                              block_num[j][i] *= 2; block_num[j][next] = 0; score += block_num[j][i]; //cal_score
...
function setup() {
     smooth(8); frameRate(30); pixelDensity(displayDensity()); let sans; textFont(sans); createCanvas(800, 600);
     background("#FAF8F0");
...
function preload() {
     sans = loadFont("Sans.ttf");
}
function start_game(reset) {
     lose = 0; score = 0; appear[0] = - 1; appear[1] = - 1; animation_start = 0; count = 0; //reset
     if (reset == 1) {
...
function draw() {
     let steps; show_block(); show_score(); if (check_can_move() == 0) lose = 1;
}
