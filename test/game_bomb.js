let numbers = Array.from(new Array(9), () => new Array(9)); //0 green or 1 red
...
let status = Array.from(new Array(9), () => new Array(9)); //0,1 click once,2 click twice
let lose = 0;
let a = 0;
...
function mousePressed() {
    if (mouseY >= 60 && mouseY <= 600) {
        ...
        let yourString = "";
        let allChars = new Array(yourString.length);
...
function draw_boom_text() {
    for (let e = 0; e < boom_number_text_count; e += 3) {
        textAlign(CENTER);
        textSize(40);
        fill(0); //println(boom_number_text[e],boom_number_text[e+1] *60,boom_number_text[e+2]*60);
        text(
            boom_number_text[e + 2],
            boom_number_text[e] * 60 + 30,
            boom_number_text[e + 1] * 60 + 105
        );
    }
}
